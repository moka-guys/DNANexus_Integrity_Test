# Path to the dx-toolkit environment
DX_ENV=/etc/profile.d/dnanexus.environment.sh
# DNANexus project that should be used for the run
PROJECT=project-F85Pb600gBbb98vQ0fgFKb1P
# Path to the DNANexus workflow to run (assumed to be in above project)
WORKFLOW=Workflows/GATK3.5_DNANexus_Integrity_Test
# Path to the truth set output folders (assumed to be in above project)
TRUTH_SET=Truth_Set